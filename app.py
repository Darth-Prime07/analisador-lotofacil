# app.py (v3.5 - Versão Completa e Funcional)
...
if modo_app == "Gerador de Jogos":
    ...
    if st.sidebar.button("Rodar Simulação", type="primary", use_container_width=True):
        ...
        st.info("Ajuste as configurações e clique em 'Rodar Simulação' para começar.")
        ...
        if estrategia_obj:
            ...
            with col2:
                ...
                if st.button("Rodar Simulação de Desempenho para esta Estratégia"):
                    ...
                    m = resultados['metricas_gerais']
                    st.metric(f"Resultado da Simulação (100 Concursos)", f"R$ {m['lucro_total']:.2f}", f"{m['roi_total']:.2f}% ROI")
